...
export default async function Page({ params: { attemptId, index } }: PageProps) {
  ...
  const attempt = await prisma.examAttempt.findFirst({
    where: { id: attemptId, userId: currentUserId },
    include: { template: true }, // 問題総数などテンプレート情報も取る
  });
  if (!attempt) return notFound();

  // 表示対象の「attemptQuestion（この試行に紐づく特定の問題）」を取得
  const attemptQuestion = await prisma.attemptQuestion.findFirst({
    where: { attemptId, orderIndex },
    include: {
      question: { include: { choices: true } }, // 選択肢まで一括取得
      answer: true, // すでに回答済みなら初期値に使う
    },
  });
